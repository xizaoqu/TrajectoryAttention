    ...
    def step_single_dgs(
        ...
        if temp_cond_latents is not None:
            ...
            for ii, tau in enumerate(index_list):
                ...
                top_mask = (torch.abs(masked_diff) <= cutoff_value) & mask_t
                top_mask = gaussian_blur(top_mask[:,:1].float()) * top_mask
                
                top_masks.append(top_mask[0,0])
                # pred_original_sample[:,tau,:,:,:][top_mask] =  FEA[top_mask]
                pred_original_sample[:,tau,:,:,:] = pred_original_sample[:,tau,:,:,:] * (1-top_mask.float()) \
                                            + FEA * top_mask.float()
        ...
        # import pdb;pdb.set_trace()
        # bool_tensor_to_gif(torch.stack(top_masks), 'top_masks3.gif')
        # 2. Convert to an ODE derivative
